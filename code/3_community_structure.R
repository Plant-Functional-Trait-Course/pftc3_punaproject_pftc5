...
comm_structure_2018 <- comm_structure_raw_2018 %>%
  ...
  pivot_longer(cols = c(cover_forbs:cover_litter), names_to = c("variable" , "functional_group"), names_sep = "_", values_to = "value") %>%
  # add height data
  ...
  bind_rows(comm_structure_raw_2018 %>%
              ...
              pivot_longer(cols = c(cover_shrub_layer:cover_bottom_layer), names_to = c("functional_group"), values_to = "value") %>%
              mutate(functional_group = str_remove(functional_group, "cover\\_"),
                     variable = "cover")) %>%
  mutate(functional_group = if_else(functional_group == "open", "bare_ground", functional_group),
         course = "PFTC3") %>%
  filter(!is.na(value))
...
comm_structure_2019 <- comm_structure_raw_2019 %>%
  ...
  pivot_longer(cols = c(cover_graminoids:cover_solid_litter), names_to = c("variable" , "functional_group"), names_sep = "_", values_to = "value") %>%
  # add height data
  bind_rows(comm_structure_raw_2019 %>%
              ...
              mutate(variable = str_remove(variable, "\\_cm"),
                     functional_group = case_when(variable == "bryophyte_depth" ~ "bryophyte",
                                                  TRUE ~ "vegetation"))) %>%
  mutate(functional_group = case_when(functional_group == "open" ~ "bare_ground",
                                      functional_group == "woody" ~ "shrub",
                                      functional_group == "solid" ~ "solid_litter",
